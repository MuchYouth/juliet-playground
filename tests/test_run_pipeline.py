...
def test_full_subcommand_defaults_to_trace_first_orchestration(monkeypatch, tmp_path):
    ...
    def fake_run_stage02b_epic002(**kwargs):
        called.append('02b_epic002_classification')
...
def test_full_subcommand_enable_pair_runs_pair_orchestration(monkeypatch, tmp_path):
    module = load_module_from_path(
        'test_run_pipeline_full_enable_pair',
    ...
    monkeypatch.setattr(module, 'export_trace_dataset_from_pipeline', fail_if_called)
    monkeypatch.setattr(module, 'export_vuln_patch_dataset', fail_if_called)
    ...
    run_dir = pipeline_root / 'run-pair'
    ...
    assert not (run_dir / '07_dataset_export' / 'vuln_patch' / 'summary.json').exists()
...
def test_full_subcommand_returns_failure_on_step_error(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        module, 'run_step02b_epic002_classification', lambda **kwargs: {'step': '02b_epic002'}
    )
    monkeypatch.setattr(
...
def test_full_subcommand_defaults_to_pruning_single_child_flows(monkeypatch):
    ...
    assert result == 0
    assert captured['config'].enable_pair is False
    assert captured['config'].prune_single_child_flows is True


def test_full_subcommand_enable_pair_sets_pair_mode(monkeypatch):
    module = load_module_from_path(
        'test_run_pipeline_enable_pair_flag',
        REPO_ROOT / 'tools/run_pipeline.py',
    )
    captured: dict[str, object] = {}
    ...
    result = run_module_main(module, ['full', '121', '--enable-pair'])

    assert result == 0
    assert captured['config'].enable_pair is True


def test_removed_full_subcommand_options_are_rejected():
    module = load_module_from_path(
        'test_run_pipeline_removed_full_options',
        REPO_ROOT / 'tools/run_pipeline.py',
    )
    for option in [
        '--disable-pair',
        ...
        argv = ['full', '121', option]
        if option == '--dedup-mode':
            argv.append('row')
        with pytest.raises(SystemExit):
