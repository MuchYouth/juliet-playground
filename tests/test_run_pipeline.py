...
def test_full_subcommand_defaults_to_pruning_single_child_flows(monkeypatch):
    ...
    assert captured['config'].use_epic002_for_02a is True
...
def test_full_subcommand_disable_epic002_for_02a_uses_legacy_path(monkeypatch):
    module = load_module_from_path(
        'test_run_pipeline_disable_epic002_for_02a_flag',
        REPO_ROOT / 'tools/run_pipeline.py',
    )
    captured: dict[str, object] = {}
    ...
    result = run_module_main(module, ['full', '121', '--disable-epic002-for-02a'])

    assert result == 0
    assert captured['config'].use_epic002_for_02a is False


def test_full_subcommand_can_disable_epic002_and_keep_legacy_order(monkeypatch, tmp_path):
    module = load_module_from_path(
        'test_run_pipeline_full_disable_epic002_runtime',
        REPO_ROOT / 'tools/run_pipeline.py',
    )
    ...
    monkeypatch.setattr(module._stage02b_flow, 'run_stage02b_flow', fake_run_stage02b_flow)
    monkeypatch.setattr(module._stage02b_epic002, 'run_stage02b_epic002', fail_epic002)
    monkeypatch.setattr(
        module._stage03_infer, 'run_infer_and_signature', fake_run_infer_and_signature
    )
    monkeypatch.setattr(
